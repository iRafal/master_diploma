...
namespace NeuralNetworkSystemBLL
{
    public class NeuralNetwork : INeuralNetwork
    {
        ...
        private List<Weight> GetWeighsForNeuronOutput(INeuron neuron)
        {
            ...
            return
                nextLayer.Neurons.Where(n => !n.IsThreshold)
                    .Select(
                        nextNeuron =>
                            WeightRepository.GetWeight(currentLayerIndex, nextLayerIndex, currentNeuronIndex,
                                nextNeuron.ElementIndex))
                    .ToList();
        }
        ...
        private void FrontIteration(ILearningSample sample)
        {
            ...
            {
                neuron.Error = sample.OutputLayer.Neurons.Single(n => n.ElementIndex == neuron.ElementIndex).Value -
                               neuron.Value;
            }
        }
        ...
        private void BackIteration()
        {
            ...
            for (var i = layersCount - 1; i > 0; i--)
            {
                ...
                for (var j = 0; j < neuronsCount; j++)
                {
                    ...
                    {
                        LearningFunctions.GetHiddenGradient(neuronsArray[j], GetWeighsForNeuronOutput(neuronsArray[j]),
                            Layers[i + 1].Neurons.Where(n => !n.IsThreshold).ToList());
                    }
        ...
        private void WriteErrorsToFile(double percentage)
        {
            try
            {
                var filePath = @"D:\\" + Layers.Count() + "Errors.txt";

                using (var sw = new StreamWriter(File.Open(filePath, System.IO.FileMode.Append)))
                {
